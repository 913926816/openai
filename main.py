...
# 注册插件
@register(name="openAi-Api一键修改", description="一键修改API为openAI", version="0.2", author="小馄饨")
class KeyConfigPlugin(BasePlugin):

    def __init__(self, host: APIHost):
        self.config_path = "data/config/provider.json"
        self.llm_models_source = "plugins/openAI-Api/llm-models.json"
        self.llm_models_target = "data/metadata/llm-models.json"
        self.user_states = {}  
        self.host = host
        self.fixed_api_url = "https://openai.zhil.cc/v1"  # Add fixed API URL
        ...
        if sender_id in self.user_states:
            ...
            if current_state['step'] == 0:  # 处理选择操作
                ...
                if not msg.startswith('sk-'):
                    ctx.add_return("reply", ["API Key格式不正确，请重新输入\n(格式应为: sk-xxxxxxxx)\n如果你不知道API Key，请点击https://openai.zhil.cc/ 购买API Key\n\n输入0返回上一步，输入10结束设定（不保存）"])
                    return
